...
{
public:
    Station(const std::string &name, double averageTemperature)
        : m_name(name), m_averageTemperature(averageTemperature), m_generator(m_device()), m_distribution(averageTemperature, 10.0)
    {
    }
...
std::size_t ParseStation(Station& station, char* buffer, int bufferLength) noexcept
{
    const double temperature = station.GenerateTemperature();
    const std::string_view name = station.GetName();
...
int main()
{
    const int numberOfStations = sizeof(stations) / sizeof(Station);
    const int lines = 1'000'000'000;
    std::filesystem::path directory = std::filesystem::current_path() / "data";
    std::filesystem::path filePath = directory / "data.csv";
    ...
    indicators::ProgressBar progressBar{indicators::option::BarWidth{50},
                                        indicators::option::Start{"["},
                                        indicators::option::Fill{"="},
                                        indicators::option::Lead{">"},
                                        indicators::option::Remainder{" "},
                                        indicators::option::End{"]"},
                                        indicators::option::PostfixText{"Generating data..."},
                                        indicators::option::ForegroundColor{indicators::Color::yellow},
                                        indicators::option::FontStyles{std::vector<indicators::FontStyle>{indicators::FontStyle::bold}},
                                        indicators::option::ShowElapsedTime{true},
                                        indicators::option::ShowRemainingTime{true},
                                        indicators::option::PrefixText{"Progress: "},
                                        indicators::option::MaxProgress{lines / 1000}};
    char buffer[50] = {0};
    for (int i = 1; i < lines; ++i)
    {
        Station &station = stations[i % numberOfStations];
        const std::size_t length = ParseStation(station, buffer, 50);
